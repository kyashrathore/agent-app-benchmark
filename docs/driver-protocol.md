...
`session-navigation-v1` and `workspace-panel-v2` also receive the authoritative ordered load definitions at `scenarioDefinition.cases.panelLoads`. Each object is `{id, expandedDirectoryCount, retainedFileTabCount, expandedReviewFileCount}`. The canonical values are light `2/2/1`, moderate `8/3/6`, and heavy `16/4/24`. Review still loads all 24 canonical changed-file identities and their complete, non-truncated authoritative data. `expandedReviewFileCount` is logical application state, not a required concurrent DOM-node count. An execute case carries only `loadProfile`; the driver resolves it against this prepared array and seeds the exact retained UI state before starting the action clock.

Review may use its ordinary production virtualization. During untimed setup the driver may scan or scroll the real production surface to attest all 24 identities, complete data, and exact logical expansion count, then it must restore the action's required canonical start position before trusted `pointerdown`. Readiness never requires offscreen Review bodies to be mounted concurrently. At the timed endpoint, the full authoritative model and exact logical state must be applied, and every Review body currently materialized in the canonical viewport must be painted and interactive.
