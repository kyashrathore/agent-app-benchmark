...
function renderIndex(model) {
  ...
  const sessionSwitchComparison = pairedScenarioSection(model, "sessionSwitch", "Session switching", renderSessionSwitchComparison);
  ...
  const headline = renderHeadline(model);
...
function renderApp(model, app) {
  ...
  if (app.sessionSwitch) sections.push(renderSessionSwitchComparison([app], model));
...
/**
 * Headline results: the few numbers a reader needs first, as plain paired
 * values with a winner line. Cards appear only for paired-valid scenarios
 * with finite values on both sides — anything invalid or unpaired keeps its
 * full disclosure in the sections below instead of a compressed card.
 */
function renderHeadline(model) {
  if (model.apps.length !== 2) return "";
  const [left, right] = model.apps;
  const paired = (property) => {
    const scenarioId = left[property]?.scenario.id;
    return scenarioId && right[property] && model.compatibility[scenarioId]?.status === "valid";
  };
  const cards = [];
  if (paired("sessionSwitch")) {
    const lane = (app, key) => app.sessionSwitch.derivation.summary[key];
    cards.push(headlineCard(model.apps, "Session switch", "1 MiB session · within workspace · cold surface", [lane(left, "within-workspace-cold"), lane(right, "within-workspace-cold")], "ms", "faster"));
    cards.push(headlineCard(model.apps, "Return to visited session", "1 MiB session · within workspace · warm surface", [lane(left, "within-workspace-warm"), lane(right, "within-workspace-warm")], "ms", "faster"));
  }
  if (paired("appStart")) {
    const start = (app) => app.appStart.derivation.summary["new-application-state"];
    cards.push(headlineCard(model.apps, "Cold app start", "process spawn → painted, input-ready", [start(left), start(right)], "ms", "faster"));
  }
  if (paired("sessionSwitch")) {
    const rss = (app) => app.resourceWindows?.windows?.active?.rssP95MiB;
    cards.push(headlineCard(model.apps, "Active memory", "summed process-family RSS under session load", [rss(left), rss(right)], "MiB", "lower"));
    const growth = (app) => app.resourceWindows?.retainedRssGrowthMiB;
    cards.push(headlineCard(model.apps, "Retained memory growth", "ending idle − baseline idle", [growth(left), growth(right)], "MiB", "lower"));
  }
  const rendered = cards.filter(Boolean).join("");
  if (!rendered) return "";
  return `<section class="headline" aria-label="Headline results"><div class="headline-grid">${rendered}</div><p class="headline-note">Nearest-rank p95 · lower is better. Full distributions, validity counts, frame health, and every remaining metric follow below.</p></section>`;
}

function headlineCard(apps, title, context, metrics, unit, word) {
  const values = metrics.map((metric) => metricValue(metric, "p95"));
  if (!values.every(Number.isFinite)) return "";
  const winnerIndex = values[0] === values[1] ? -1 : values[0] < values[1] ? 0 : 1;
  const ratio = winnerIndex >= 0 && values.every((value) => value > 0) ? Math.max(...values) / Math.min(...values) : null;
  const verdict = winnerIndex < 0 ? "Tie" : `${escapeHtml(apps[winnerIndex].name)}${ratio ? ` · ${formatRatio(ratio)}× ${escapeHtml(word)}` : ""}`;
  const counts = (metric) => Number.isFinite(metric?.valid) ? ` · ${metric.valid}/${metric.attempted}` : "";
  const value = (index) => `<div class="headline-value app-tone-${index}${index === winnerIndex ? " win" : ""}"><i aria-hidden="true"></i><b>${formatWithUnit(values[index], unit)}</b><small>${escapeHtml(apps[index].name)}${counts(metrics[index])}</small></div>`;
  return `<article class="headline-card"><h3>${escapeHtml(title)}</h3><p>${escapeHtml(context)}</p>${value(0)}${value(1)}<p class="headline-verdict">${verdict}</p></article>`;
}
 ...
 */
function renderSessionSwitchComparison(apps, model = {}) {
  const statistic = model.primaryStatistic ?? "p95";
  const statisticLabel = statistic.toUpperCase();
  const lanes = [
    ["within-workspace-cold", "Within workspace", "cold surface · 1 MiB session"],
    ["within-workspace-warm", "Within workspace", "warm (previously rendered) surface · 1 MiB session"],
    ["across-workspaces-cold", "Across workspaces", "cold surface · 1 MiB session"],
    ["across-workspaces-warm", "Across workspaces", "warm surface · 1 MiB session"],
  ];
  const rows = lanes
    .filter(([key]) => apps.some((app) => app.sessionSwitch.derivation.summary[key]))
    .map(([key, flow, context]) => navigationMatrixRow(flow, context, apps, (app) => app.sessionSwitch.derivation.summary[key], statistic))
    .join("");
  const trendSeries = apps.map((app, colorIndex) => ({
    label: app.name,
    colorIndex,
...
function workspaceInteractionCells(interaction, action, statistic, frameStatistic) {
  ...
  return workspaceMetricCell(response, statistic, "ms", responseLabel);
}
