...
function renderWorkspacePanelTrend(lines, summary) {
  ...
  lines.push("", "Each row is one ordinary user action. The declared logical panel state is seeded before timing; setup is excluded. Review requires complete non-truncated 24-file data and exact logical expansion counts, while production-virtualized offscreen bodies need not be mounted. Open-file begins with authoritative target bytes warm through the production data path, but with its target tab and preview never mounted; the measured input owns first surface creation and paint. The current viewport must be painted and interactive. Opening reports shell, animation, data, paint, and interactive milestones.", "");
}
